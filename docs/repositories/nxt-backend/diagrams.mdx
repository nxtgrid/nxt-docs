...
These diagrams still show the previous service split (one large HTTP app, in-process meter messaging, after-effects). Meter command delivery is now [`nxt-device-messaging`](../nxt-device-messaging). What you deploy from this repo is [`api`](./api) and [`worker`](./worker).
:::
