...
- Typical App Platform shape: `api` and `worker` built from **this** repo (Node.js buildpack). Add `nxt-device-messaging` as a third component only when metering is on. Hub: [DigitalOcean App Platform](./nxt-backend/digital-ocean). Source also has `docs/deployment/supabase.md`.
...
- Deploy: `docs/deployment/` (hub: [DigitalOcean App Platform](./nxt-backend/digital-ocean))
- Older tree: `legacy/`
