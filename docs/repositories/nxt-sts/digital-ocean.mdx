...
If [nxt-device-messaging](../nxt-device-messaging) is another component in the same app, set **on that component** (not on STS):

```text
NXT_STS_URL=${nxt-sts.PRIVATE_URL}
```

Enable `{ "id": "nxt-sts" }` in device-messaging `plugins[]` only if that process should mint tokens. Device-messaging App Platform notes: [Deploy nxt-device-messaging](../nxt-device-messaging/digital-ocean).
